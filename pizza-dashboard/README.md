...
Sprint 3.7 adds completed Square POS walk-ins to the production board and lets the kitchen drag them into local service slots without modifying the original Square order.

- Pulls scheduled pickup orders and completed Square POS sales for the date selected in the dashboard.
...
- Hides drink-category items from the production board.
...
- Detects completed `SQUARE_POS` orders with no pickup fulfillment as walk-ins.
...
- Stores walk-in slot assignments in SQLite so a Square refresh preserves them.
...
A modifier whose name contains `salad` is treated as a named salad. Modifiers containing the word `Side` are tracked separately, and cookie modifiers trigger a cookie alert. Prepared dough, salad, and side counts are edited inside **Service setup** and summarized in read-only cards during service.
...
4. Separately keeps completed orders created by `SQUARE_POS`, with no pickup fulfillment, whose local `closed_at` date matches the selected service date. `created_at` is used only when `closed_at` is absent.
...
while retaining a cached fallback if the live request fails. Each pizza row also includes an eight-minute bake timer. Timer state is local to that browser/device and survives normal page refreshes. Completed POS orders first appear in the Unscheduled walk-in lane; drag them onto a service slot to mirror the pickup time written on the physical ticket.
