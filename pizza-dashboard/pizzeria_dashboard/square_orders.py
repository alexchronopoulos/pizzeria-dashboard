    ...
    *,
    ...
    if callable(list_payments):
        try:
            ...
            warnings.append(
                "Receipt numbers could not be loaded. Orders were synced without "
                f"them. Square said: {exc}"
            )
...
def convert_square_orders(
    ...
    *,
    ...
    for raw_order in raw_orders:
        ...
        if walk_in_at is None or not _is_square_pos_order(raw_order):
            continue
...
def _is_square_pos_order(raw_order: Mapping[str, object]) -> bool:
    if _creation_product(raw_order) == "SQUARE_POS":
        return True

    # Older order documents might not expose creation_source. Fall back to a
    # conservative name check instead of treating every non-online sale as a walk-in.
