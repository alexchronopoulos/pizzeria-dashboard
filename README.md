...
- Summarizes pizza add-on modifiers for ingredient prep and hides elapsed pickup slots from today's operational selectors. Prep View hides empty slots only; populated past orders remain in place so active timer positions do not jump during service. New-order notifications and the right-side timer rail use separate, bounded overlay lanes, and refreshes restore the service viewport only after both rails have settled.
